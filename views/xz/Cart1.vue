<template>
  <div>
    <!-- <h3>Cart.vue 购物车</h3> -->
    <!-- 1：顶部全选复选框 -->
    <div class="gw">
      <p>购物车</p>
    </div>
    <div
      class="selectall"
      style="padding-top:54px;height:30px;border:0px solid #ccc;margin-bottom:-20px;"
    >
      全选&nbsp;&nbsp;
      <input type="checkbox" v-model="allcb" @change="selectAll" />
    </div>
    <!-- 2：购物车商品列表 -->
    <div class="cartItem" v-for="(item,i) of list" :key="i">
      <div class="leftImgTxt">
        <input type="checkbox" v-model="item.cb" @change="changeItem" />
        <!-- <img :src=" 'http://127.0.0.1:4000/'+item.pic" width="90%"> -->
        <div class="title">{{item.lname}}</div>
        <div class="title2">{{item.price}}￥</div>
        <div class="title3">
          <mt-button @click="sub1(item.id)" min="1">-</mt-button>
          <span>{{item.count}}</span>
          <mt-button @click="add(item.id)">+</mt-button>
        </div>
      </div>
      <mt-button @click="del(item.id)">x</mt-button>
    </div>
    <!-- 3：一组按钮 -->
    <div class="db">
      购物车中商品的数量
      <span style="color:red">{{count}}</span>      
      <mt-button @click="delm">删除选中商品</mt-button>
      <mt-button @click="delall">清空购物车</mt-button>
    </div>
  </div>
</template>
<script>
export default {
  data() {
    return {
      list: [], //购物车购物列表信息
      allcb: false //全选按钮的状态
    };
  },
  computed:{
      count(){
          return this.list.length;
      }
  },
  created() {
    //组件创建成功后获取购物车列表信息
    this.loadMore();
    console.log("重新加载啦");
  },
  methods: {
    sub1(d) {
      var url = "sub1";
      var params = {
        params: {
          id: d
        }
      };
      // console.log(item.count)
      console.log(params);
      this.axios.get(url, params).then(res => {
        if (res.data.code == 1) {
          // this.$router.go(0);
          this.loadMore();
        }
      });
    },
    add(i) {
      // console.log(i);
      var url = "add";
      var params = {
        params: {
          id: i
        }
      };
      console.log(params);
      this.axios.get(url, params).then(res => {
        if (res.data.code == 1) {
          // this.$router.go(0);
          this.loadMore();
        }
      });
    },
    delall() {
      if (this.list.length == 0) {
        this.$messagebox("消息", "当前没有可删除的商品");
        // 返回
        return;
      } else {
        this.$messagebox.confirm("是否删除所有商品?").then(res => {
          var url = "delall";
          this.axios.get(url).then(res => {
            //重新请求数据刷新页面
            this.loadMore();
            this.$toast("删除成功");
          });
        });
      }
    },
    sub() {
      // 功能：修改全局共享函数
      this.$store.commit("subFa");
    },
    delm() {
      // 功能删除选中的商品
      // 1：判断商品数量 如果数量为零 this.list
      if (this.list.length == 0) {
        // 2：显示确认框   当前没有可删除的商品
        this.$messagebox("消息", "当前没有可删除的商品");
        // 3：返回
        return;
      }
      // 4：创建变量str 目的拼接字符串 "2,3,4"
      var str = "";
      // 5：创建循坏数据遍历数组 this.list
      // 6：判断当前商品是否选中状态 cb==true
      // 获取id拼接字符串str+","
      for (var item of this.list) {
        if (item.cb) {
          str += item.id + ",";
        }
      }
      // console.log(str);
      // 7:判断：如果用户没有选中商品str.length==0
      // 8：显示确认框  请选择要删除的商品
      if (str.length == 0) {
        this.$messagebox("消息", "请选择需要删除的商品");
        return;
      }
      // 8.1:截取字符串  "2,3,"=>"2,3"
      str = str.substring(0, str.length - 1);
      // console.log(str);
      // 9：显示确认交互框 是否删除指定数据
      // 10：创建 变量 url obj
      // 11：发送ajax请求删除指定数据
      this.$messagebox.confirm("是否删除指定商品？").then(res => {
        var url = "delm";
        var obj = { id: str };
        this.axios.get(url, { params: obj }).then(res => {
          //重新请求数据刷新页面
          this.loadMore();
          this.$toast("删除成功");
        });
      });
    },
    selectAll() {
      // 功能：全选按钮状态修改操作
      // 1:获取全选按钮状态
      var cb = this.allcb;
      // 2：创建循环遍历所有商品状态赋值
      for (var item of this.list) {
        item.cb = cb;
      }
    },
    changeItem() {
      // 功能：商品状态修改操作
      // 功能一：累加商品状态为true
      var sum = 0;
      for (var item of this.list) {
        if (item.cb) sum++;
      }
      // 功能二：判断如果商品总数量与等于true总数量数量相同
      if (this.list.length == sum) {
        this.allcb = true;
      } else {
        this.allcb = false;
      }
    },
    loadMore() {
      // console.log(123);
      //发送请求购物车数据
      // 1:创建变量url保存请求服务器程序地址
      var url = "findcart";
      // 2：发送ajax请求并且获取服务器返回数据
      this.axios.get(url).then(res => {
        // 3：获取服务器返回的数据code==-2
        // 提示 请登录    跳转Login组件
        //if (res.data.code == -2) {
        if(!localStorage.getItem("uname")){    
          this.$toast("请登录");
          this.$router.push("/Login1");
          return;
        } else {
          // 4：获取服务器返回数据  code==1
          // 5：将服务器返回购物车列表保存list
          // this.list = res.data.data;
          //5.1获取服务器中购物车列表，临时保存变量rows
          var rows = res.data.data;
          // 5.2：创建循环遍历数组中每一个商品并且添加cb属性
          //     表示商品是否是选中状态
          for (var item of rows) {
            item.cb = false;
          }
          // 5.3：将新数组赋值list
          this.list = rows;
          console.log(this.list);
          // console.log(this.list);
          // 6：在模板中创建循坏显示购物车列表数据即可
          // 7:修改购物车全局数据
        //   this.$store.commit("addmCart", this.list.length);
        }
      });
    },
    del(id) {
      // console.log(id);
      //功能：删除购物车中指定的商品
      // 1：显示交互对话框
      this.$messagebox
        .confirm("是否删除指定商品？")
        .then(res => {
          // 2：如果用户选中 确认
          // 3：创建url 保存中间人商城程序的值 /del
          var url = "del";
          // 4：创建obj 保存id
          var obj = { id };
          // 5：发送ajzx请求
          this.axios.get(url, { params: obj }).then(res => {
            // 6：获取返回结果 提示 重新加载数据【刷新】
            if (res.data.code == 1) {
              this.$toast("删除成功");
              this.loadMore();
            }
          });
        })
        .catch(err => {});
    }
  }
};
</script>
<style scoped>
/*1一个商品项目元素*/
.cartItem {
  display: flex; /*弹性布局*/
  justify-content: space-between; /*子元素两端对齐*/
  align-items: center; /*子元素垂直居中*/
  border-bottom: 1px solid #ccc;
  margin-top: 25px; /*商品之间间距*/
  width: 98%;
  height: 100px;
  border: 2px solid #64212d;
}
/*左侧【商品名称与价格】*/
.leftImgTxt {
  /* display: flex; */
  align-items: center;
}
.leftImgTxt input {
  margin-top: 40px;
  margin-left: 15px;
}
.title {
  margin-left: 50px;
  margin-top: -50px;
  font-size: 20px;
}
.title2 {
  margin-left: 50px;
  margin-top: 20px;
}
.title3 {
  margin-top: -20px;
  margin-left: 260px;
}
.gw {
  display: flex; /*指定布局方式为弹性布局*/
  position: fixed; /*固定定位*/
  z-index: 999; /*显示在元素上方*/
  width: 100%; /*填满父元素*/
  justify-content: space-between; /*子元素两端对齐*/
  align-items: center; /*子元素垂直居中*/
  font-weight: bold;
  height: 48px;
  background-color: #fdfdfd;
  padding-left: 7px;
  padding-right: 7px;
  height: 48px;
  font-size: 18px;
}
/* .db{
    /* display: flex;指定布局方式为弹性布局 */
/* position: fixed;固定定位 */
/* z-index:999;显示在元素上方 */
/* padding-top: 10px; */
/* } */
</style>